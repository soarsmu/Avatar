    ...
    for epoch in range(args.epochs):
        ...
        if dev_acc >= dev_best_acc:
            ...
            output_dir = os.path.join(
                args.model_dir, str(args.count), "best")
...
def main():
    ...
    surrogate_set = []
    with open("surrogate.jsonl") as f:
        for line in f:
    ...
    for su in tqdm(surrogate_set):
        ...
        if args.do_train:
            train_dataset = DistilledDataset(
                args, config.vocab_size, args.train_data_file, logger)
            train_sampler = RandomSampler(train_dataset)
            train_dataloader = DataLoader(
                train_dataset, sampler=train_sampler, batch_size=args.train_batch_size)

        eval_dataset = DistilledDataset(
            args, config.vocab_size, args.eval_data_file, logger)
